...
function convertStringToDate(date) {
  let dateString = date.slice(7).replace(/(\d{2})-(\d{2})-(\d{4})/, '$3-$1-$2');
...
function mailCount(emailData) {
  ...
  let dates = emails.map(email => convertStringToDate(email.split('#/#')[2]));
  
  dates.sort(compareDates);
  
  console.log(`Count of Email: ${count}`);
  console.log(`Date Range: ${getDateRange(dates)}`);
}
